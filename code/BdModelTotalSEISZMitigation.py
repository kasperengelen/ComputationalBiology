...
class BdModelTotalSEISZ(CompartmentalModel):
    """
        A compartmental model for the population, both terrestrial and riparian. This model
        uses the SEIS(Z) compartment configuration.
    """
    ...
    def __init__(self, rip_init_pop, ter_init_pop, plot_all = False):
        ...
        super(BdModelTotalSEISZ, self).__init__(init_time = 1998, y_max = 9000000, y_min = None)

        # 1-1-1998 => 1-1-2005: 7 years
        # here we do 10 years since we want to predict the effect
        self.simulation_length = 10 * 365
        ...
        param.lamb_ter = 0.6864529058116232

        # absolute grownth of the riparian population
        # OLD: 1.2242897805796555
        param.lamb_rip = 1.2085370741482966
    ...
    def updateTotals(self):
        ...
        comp.total_rip = (comp.S_rip + comp.E_rip + comp.I_rip) * self.rip_init_pop
        ...
        comp.total_ter = (comp.S_ter + comp.E_ter + comp.I_ter) * self.ter_init_pop

    def update(self):
        super(BdModelTotalSEISZ, self).update(1.0 / 365.0)
        ...
        # on other days we run the normal formulas
            ...
            new.S_ter = old.S_ter - (param.beta * old.S_ter * old.I_ter) * dt + param.lamb_ter * dt + (param.gamma * infl_z(old.Z_ter, param.k_ter) * old.I_ter) * dt
            new.E_ter = old.E_ter + (param.beta * old.S_ter * old.I_ter) * dt - (param.sigma * infl_z(old.Z_ter, param.k_ter) * old.E_ter) * dt
            new.I_ter = old.I_ter + (param.sigma * infl_z(old.Z_ter, param.k_ter) * old.E_ter) * dt - (param.gamma * infl_z(old.Z_ter, param.k_ter) * old.I_ter) * dt - (param.alpha * infl_z(old.Z_ter, param.k_ter) * old.I_ter) * dt
            new.Z_ter = old.Z_ter + (param.phi * old.I_rip) * dt - (param.mu * old.Z_ter) * dt

            ## riparian ##
            new.S_rip = old.S_rip - (param.beta * old.S_rip * old.I_rip) * dt + param.lamb_rip * dt + (param.gamma * infl_z(old.Z_rip, param.k_rip) * old.I_rip) * dt - (param.eps * infl_z(old.Z_rip, param.k_rip)) * dt
            new.E_rip = old.E_rip + (param.beta * old.S_rip * old.I_rip) * dt - (param.sigma * infl_z(old.Z_rip, param.k_rip) * old.E_rip) * dt + (param.eps * infl_z(old.Z_rip, param.k_rip)) * dt
            new.I_rip = old.I_rip + (param.sigma * infl_z(old.Z_rip, param.k_rip) * old.E_rip) * dt - (param.gamma * infl_z(old.Z_rip, param.k_rip) * old.I_rip) * dt - (param.alpha * infl_z(old.Z_rip, param.k_rip) * old.I_rip) * dt
...
def main():
    ...
    # whether or not to plot the S,E,I compartments. Set to False to only plot the total
    ...
    # run model
    model = BdModelTotalSEISZ(rip_init_pop = rip_init_pop_abs, ter_init_pop = ter_init_pop_abs, plot_all = PLOT_ALL_COMPARTMENTS)

    # set zoospore decay rate
    model.parameters.mu = 120
    #model.parameters.mu = 365
